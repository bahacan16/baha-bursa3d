...
- **Street View pilotu (Mertkent 2. Etap):** `fetch-streetview.yml` (anahtar GitHub secret `GOOGLE_STREETVIEW_KEY`, Street View Static API) cephe gören kareleri indirir (74 panorama, 246 kare, 2025-09); `scripts/bake-streetview.mjs` bunları OSM duvarlarına yansıtıp `facades.jpg/json` atlası üretir, oyun `src/worlds/osm/streetview.ts` ile duvarların 5 cm önüne kabuk olarak ekler (`?nosv=1` kapatır). Duvar başına tek panorama (paralaks hayaleti yok); dokusunda gökyüzü görünen duvar reddedilir (OSM geometrisi o duvarda gerçeğe uymuyor). Sonuç: 603 m duvarın 208 m'si fotoğraflı. Çok-görüş plane sweep (`sv-refine.mjs`, `SV_REFINE=1`) denendi, NCC zayıf (balkon/çalı/ağaç) → varsayılan kapalı.
